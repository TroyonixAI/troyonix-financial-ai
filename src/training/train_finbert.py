...
def get_device():
    ...
    logger.info("Forcing CPU usage for compatibility")
...
def prepare_dataset(df: pd.DataFrame) -> Tuple[Dataset, Dataset]:
    ...
    # Prepare for HuggingFace Trainer: rename 'label_id' to 'label' and ensure int type
    for split_df in [train_df, val_df]:
        split_df['label'] = split_df['label_id'].astype(int)
        split_df.drop(columns=['label_id'], inplace=True)
        # Optionally drop the original string label column to avoid confusion
        if 'label' in split_df.columns and split_df['label'].dtype == object:
...
def finetune_model():
    ...
    training_args = TrainingArguments(
        ...
        use_mps_device=False,  # Force CPU
